...
- **Diagnóstico drag-image (corregido tras prueba visual)**: el pipeline funciona completo en esta sesión — se ven íconos de tipo y thumbnails reales. Los casos de "placeholder blanco/incompleto" son una **carrera al INICIO del drag del lado del origen**: `DragImageBits` es un snapshot que Explorer toma antes de `DoDragDrop`; si la extracción del thumbnail no terminó, congela lo que haya y **ningún drop target puede completarlo después** (la drag-image window pertenece al origen; el bitmap no se re-renderiza a mitad de drag). Mismo comportamiento al soltar sobre cualquier app.
- **Decisiones de diseño derivadas (F1)**: (1) el dropzone renderiza su PROPIA preview (paths ya parseados del CF_HDROP + `IShellItemImageFactory` async) — inmune a la carrera; (2) cuando ViennaBar es origen (drag-out del tree/stack), pre-renderizar el thumbnail antes de `DoDragDrop` vía `IDragSourceHelper::SetDragImage` — nosotros controlamos el snapshot.
